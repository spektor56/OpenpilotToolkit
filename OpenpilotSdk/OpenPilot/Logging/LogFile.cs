...
namespace OpenpilotSdk.OpenPilot.Logging
{
    public sealed class LogFile
    {
        public static async Task<IEnumerable<GpxWaypoint>> GetWaypointsAsync(Stream fileStream, CompressionAlgorithm compressionAlgorithm)
        {
            ...
            Stream decompressedStream = compressionAlgorithm switch
            {
                CompressionAlgorithm.BZip2 => new BZip2InputStream(fileStream),
                CompressionAlgorithm.ZStd => new ZstdSharp.DecompressionStream(fileStream),
                _ => fileStream
            };

            using (decompressedStream)
            {
                var pump = new FramePump(decompressedStream);
                ulong? lastTime = null;
                pump.FrameReceived += frame =>
                {
                    ...
                    var test = reader.LogMonoTime;
                    if(lastTime != null && test < lastTime)
                    {
                        Debug.Print("Out of order timestamp");
                    }
                    lastTime = test;
